"""Fetch a zone sheet's Transport/Sante tab as a PDF, on demand, from the office
review portal.

The API fetches and stores the sheet at record time, but that can miss on a
transient Google hiccup or a serverless timeout. The review portal is online and
the sheets are link-shared, so it can simply re-fetch from the zoneFund metadata
({sheetId, type}) — no phone re-save (which would duplicate the transaction).
"""
import re
import base64
import urllib.request

# Transport is gid 0 in every zone sheet; these are the resolved Sante gids.
KNOWN_SANTE_GID = {
...
def _resolve_tab_gid(sheet_id, tab_name):
    """Scrape the sheet's tab list (no auth for link-shared sheets)."""
...
def _resolve_gid(sheet_id, ztype):
    t = (ztype or "").lower()
    if t == "transport":
        return 0                                   # first tab in every zone sheet
    if t != "sante":
        return None
...
def fetch_pdf(sheet_id, ztype):
    """PDF bytes for the tab, or b'' on any failure (never raises)."""
    ...
    try:
        ...
        with _open(_export_url(sheet_id, gid)) as r:
            if "pdf" not in (r.headers.get("Content-Type", "") or ""):
                return b""                          # an HTML page means it isn't shared
            return r.read() or b""
    except Exception:
        return b""
